...
void RenderThreadImpl::Init(
  ...
  // Set maximum skia-font-cache size for background tab (will be executed in idle time)
  skia_background_font_cache_size_ = -1;
  if (command_line.HasSwitch(switches::kSkiaBackgroundFontCacheSizeKb)) {
    int skia_background_font_cache_size;
    std::string string_value =
        command_line.GetSwitchValueASCII(switches::kSkiaBackgroundFontCacheSizeKb);
    if (base::StringToInt(string_value, &skia_background_font_cache_size))
      skia_background_font_cache_size_ = skia_background_font_cache_size * 1024;
    else
      LOG(WARNING) << "Failed to parse switch " <<
                      switches::kSkiaBackgroundFontCacheSizeKb  << ": " << string_value;
  }
...
void RenderThreadImpl::ClearExtensions() {
  WebScriptController::clearExtensions();
}
...
void RenderThreadImpl::IdleHandler() {
  ...
  if (skia_background_font_cache_size_ >= 0 &&
      font_cache_used > size_t(skia_background_font_cache_size_)) {
    // Purge Skia font cache below than 'max_background_skia_font_cache_size_'.
    // if the value is '0' it will try to purge font cache completely.
    size_t font_cache_limit =
        SkGraphics::SetFontCacheLimit(skia_background_font_cache_size_);
...
scoped_refptr<ContextProviderCommandBuffer>
RenderThreadImpl::SharedMainThreadPluginContextProvider() {
  DCHECK(IsMainThread());
  if (shared_main_thread_plugin_contexts_ &&
      shared_main_thread_plugin_contexts_->ContextGL()
              ->GetGraphicsResetStatusKHR() == GL_NO_ERROR)
    return shared_main_thread_plugin_contexts_;

  scoped_refptr<gpu::GpuChannelHost> gpu_channel_host(EstablishGpuChannelSync(
      CAUSE_FOR_GPU_LAUNCH_RENDERER_SHARED_MAIN_THREAD_CONTEXT));
  if (!gpu_channel_host) {
    shared_main_thread_plugin_contexts_ = nullptr;
    return nullptr;
  }

  bool support_locking = false;
  shared_main_thread_plugin_contexts_ = CreateOffscreenContext(
      std::move(gpu_channel_host), gpu::SharedMemoryLimits(), support_locking,
      command_buffer_metrics::RENDERER_MAINTHREAD_CONTEXT,
      gpu::GPU_STREAM_DEFAULT, gpu::GpuStreamPriority::NORMAL);
  if (!shared_main_thread_plugin_contexts_->BindToCurrentThread())
    shared_main_thread_plugin_contexts_ = nullptr;
  return shared_main_thread_plugin_contexts_;
}
...
#endif
#if defined(OS_WEBOS)
    IPC_MESSAGE_HANDLER(ViewMsg_NotifyLocaleChanged, OnNotifyLocaleChanged)
#endif
...
#if defined(OS_WEBOS)
void RenderThreadImpl::OnNotifyLocaleChanged(const std::string& new_locale) {
  if (GetContentClient()->renderer())
    GetContentClient()->renderer()->NotifyLocaleChanged(new_locale);
}
#endif
